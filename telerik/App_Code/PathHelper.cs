...
public class PathHelper
{
    ...
    public static string TransformPath(string path)
    {
        return ConfigurationManager.AppSettings["FileStoragePath"] +
           ReverseMapPath(path).Replace("/", "\\").Replace("~", "").Replace("\\\\", "\\");
    ...
    public static string ReverseMapPath(string path)
    {
        ...
        string res = string.Format("{0}", path.Replace(appPath, ""));
        res = res.Replace(ConfigurationManager.AppSettings["FileStoragePath"], "");
        res = res.Replace("\\", "/");
        if (!res.StartsWith("/"))
            res = "/" + res;
        return res;
