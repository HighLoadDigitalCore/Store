...
*/
    ...
    public static string[] TelerikPathRelation
    {
        get
        {
            return new string[]
                    {
                        "Editor/", ConfigurationManager.AppSettings["FileStoragePath"]  + "Editor"
                    };
        }
    }
    ...
    {
        //this._itemHandlerPath = "/telerik/FileSystemHandler.ashx";

        this.mappedPathsInConfigFile = new Dictionary<string, string>();
        var pairs = TelerikPathRelation;
    ...
    {   // 'virtualPath' contains value similar to:  "/MyCusomRootDir/"
        ...
        */
        ...
        foreach (KeyValuePair<string, string> mappedPath in MappedPaths)
        {
            //if (mappedPath.Key.Replace(PhysicalPathSeparator, PathSeparator).EndsWith(virtualPath, StringComparison.CurrentCultureIgnoreCase))
    ...
    private FileItem[] GetFiles(string virtualPath)
    {
        ...
        try
        {
            ...
            foreach (string filePath in files)
            {
                ...
                string url = "/content/" + virtualPath + fileInfo.Name;
                ...
                fileItem.Attributes.Add("CreationTime", fileInfo.CreationTime.ToString("dd.MM.yyyy HH:mm:ss"));
				string ext = Path.GetExtension(filePath);
				if (ext.Length > 0)
					ext.Substring(1);
                var imagExts = new[] { "jpg", "gif", "png", "jpeg", "bmp" };
                string colText = "";
                if (imagExts.Contains(ext))
                {
                    using (Stream stream = File.OpenRead(filePath))
                    {
                        using (Image sourceImage = Image.FromStream(stream, false, false))
                        {
                            colText = sourceImage.Width + "х" + sourceImage.Height + "px";
                        }
                    }
                }
                else
                ...
                fileItem.Attributes.Add("ImageSize", colText);
        ...
        return
            filesItems.OrderByDescending(
                x =>
                DateTime.ParseExact(x.Attributes["CreationTime"], "dd.MM.yyyy HH:mm:ss", CultureInfo.CurrentCulture))
                      .ToArray();
    }
