    ...
    public static class Extensions
    {
        ...
        public static object ToSuitableType(this object inp)
        {
            ...
            if (int.TryParse((string)inp, out ret))
                return (object)ret;
            if (inp != null && (inp.ToString() == "null" || inp.ToString() == String.Empty))
            {
                return null;
            }
            return (object)inp;
        ...
        {
            return new DB().Users.FirstOrDefault(x => x.UserId == context.GetCurrentUserUID());
        }
        ...
        public static string ToNiceDigit(this decimal value)
        {
            ...
            if (SiteSetting.Get<bool>("ShowCopecks"))
            {
            ...
            {
                return (Math.Round(value)).ToString("### ### ### ###").Trim();
            }
            return integer;
        }
        ...
        public static MvcHtmlString ActionLinkQuery(this HtmlHelper html, string linkText, string actionName, string controllerName, IEnumerable<string> queryFilter, RouteValueDictionary routeValues, object htmlAttributes)
        {
            ...
            {
                if (queryFilter.Select(x=> x.ToLower()).Contains<string>(key.ToLower()) && !dictionary.Keys.Select(x=> x.ToLower()).Contains(key.ToLower()))
                {
            ...
            {
                if (!dictionary.Keys.Select(x=> x.ToLower()).Contains(pair.Key.ToLower()))
