...
$().ready(function () {
    setTimeout(function() {
        ...
        */
        ...
        loadTextTruncate();
        loadCheckBo();
        $('.small-icons .cat-item span').height('auto');
        $('.small-icons .cat-item span').css('padding-top', 0).css('transition', 'initial');
        $('.small-icons .cat-item span').each(function () { $(this).css('margin-top', (($(this).parent().height() - $(this).outerHeight()) / 2)); });
        setTimeout(function() {
            $('.small-icons .cat-item span').css('transition', "all 0.7s ease 0s");
        }, 200);


        $(".goods_item_properties .text-truncate .str-button").appendTo(".goods_item_properties");
        $('.goods_item_properties .str-button a').click(function() {
    ...
    $('.phone-ring').hover(function () {
        $(this).prev().children().toggleClass('green-border');
    });
   
    $('.tremmors').hover(function () {
        $(this).parent().toggleClass('green-border');
        $(this).parent().next().css('margin-top', '-42px;');
    });

    $('.gal-image a img').mouseover(function () {
...
/*
    ...
    $('.gal-image a').click(function () {
        $('.theme_main_preview').attr('href', $(this).attr('href'));
        $('.theme_main_preview img').attr('src', '/Master/ru/Image/Resize?skiplogo=False&filePath=' + $(this).attr('href').replace('/', '%2F') + '&padding=0&maxWidth=200&maxHeight=200&vertalign=center');
...
function loadCheckBo() {
    $('.compc2, .compare.for-idiots').each(function () {
        if (!$(this).find('.cb-inner').length) {
            $(this).checkBo();
        }
    });
...
function loadTextTruncate() {
    $('.text-truncate').stTruncate({ maxHeight: 300, maxHeightCut: 250, addBlockStyleClass: 'seoTextOpenButton', moreText: "Развернуть", lessText: "Свернуть" });
}
function cacheSprite() {
    var num = Math.ceil($('#leaflist .picbox').length / 150);
    for (var i = 0; i < num; i++) {
        $('<img/>')[0].src = '/Master/ru/Image/CatalogMenuSprite?group=' + i;
    }
...
function loadSorter() {
    var c = $.cookie('ProductOrder');
    if (c) {
        $('.sorter[arg^="' + c.replace('Desc', '') + '"]').addClass('act');

    }
    $('.sorter').click(function () {
        $.cookie('ProductOrder', $(this).attr('arg'), { expires: 365, path: '/' });
...
function showCartDialog(id, count) {
    ...
    $.get('/Master/ru/ShopCart/PopupDialog', {id:id, count:count, needchange: false}, function(d) {
        ...
        /*
                setTimeout(function() {
                    $('#el_modal_window').fadeOut(4000, function() {
                        //$('#el_modal_window').remove();
...
function calcMenuV2(obj) {
    //debugger;
    $(obj).find('a:visible').each(function () {
        var pad = ((47 - $(this).height()) / 2);
        if (pad < 0)
            pad = 0;
...
function initCatMenuV2() {
    ...
    $('.catalog-menu-2 li[arg]').hover(function () {
        if (menuV2Timer)
            clearTimeout(menuV2Timer);
        
        $(this).find('[parent="'+$(this).attr('arg')+'"]').addClass('active-sub');
        $(this).find('[parent="' + $(this).attr('arg') + '"]').show(0, function() {
            calcMenuV2(this);
        });
        $(this).find('[parent="' + $(this).attr('arg') + '"]').css('top', $(this).position().top + 'px');


    }, function () {

        $(this).find('[parent="' + $(this).attr('arg') + '"]').removeClass('active-sub');
        $(this).find('[parent="' + $(this).attr('arg') + '"]').hide();
...
function changeImg(obj) {
    $(obj).parent().parent().find('.main-img').attr('src', '/Master/ru/Image/Resize?maxWidth=180&maxHeight=180&filePath=' + $(obj).attr('path') + '&padding=0');
}
...
function updateHead() {
    $.get('/Master/ru/Cabinet/HeadCount', {}, function (d) {
        ...
        var cnt = parseInt($('#head-count .fixpanel_basket_count').text());
        /*var sum = parseInt($('#head-count .fixpanel_basket_count').attr('total')/*.replace(/\D/g, '')#1#);
        var post = '';
        if (cnt % 10 >= 2 && cnt % 10 <=4 && cnt!=12 && cnt!=13&& cnt!=14) {
            post = 'а';
        }
        if (cnt % 10 >= 5 || cnt % 10 == 0 || cnt == 12 || cnt == 13 || cnt == 14) {
            post = 'ов';
        }*/
        //$('#header .total').replaceWith('<div class="total"><b>' + cnt + '</b> товар' + post + ',<br><b>' + sum + '</b> руб.</div>');
        $('#header .fixpanel_basket_count').html(cnt);
        
        $('#header .total').replaceWith('<div class="total"><b>' + $('#head-count .fixpanel_basket_count').attr('total-fm') + '</b> руб.</div>');
        /*
                $('#header .total b:eq(0)').html();
                $('#header .total b:eq(1)').html();
        */
    });
}


function showOrderForm(obj) {
    yandexStatToOrder();
...
function setBack(maxX, maxY) {
    ...
    /*    for (var i = 0; i < maxX; i++) {
            for (var j = 0; j < maxY; j++) {
                ...
                if (i == 0) {
                    if (j == 0) {
                        cell.css({ 'background-image': 'url(/content/theme_main/img/lt.png)', 'background-repeat': 'no-repeat' });
    
                    }
                    else if (j == maxY - 1) {
                        cell.css({ 'background-image': 'url(/content/theme_main/img/lb.png)', 'background-repeat': 'no-repeat' });
    
                    } else {
                        cell.css('background-image', 'url(/content/theme_main/img/lm.png)');
    
                    }
                }
                else if (i == maxX - 1) {
                    if (j == 0) {
                        cell.css({ 'background-image': 'url(/content/theme_main/img/rt.png)', 'background-repeat': 'no-repeat', 'background-position': 'right center' });
                    } else if (j == maxY - 1) {
                        cell.css({ 'background-image': 'url(/content/theme_main/img/rb.png)', 'background-repeat': 'no-repeat', 'background-position': 'right center' });
    
                    } else {
                        cell.css({ 'background-image': 'url(/content/theme_main/img/rc.png)', 'background-repeat': 'no-repeat', 'background-position': 'right center' });
    
    
                    }
                } else {
                    if (j == 0) {
                        cell.css({ 'background-image': 'url(/content/theme_main/img/ct.png)', 'background-repeat': 'no-repeat' });
                    } else if (j == maxY - 1) {
                        cell.css({ 'background-image': 'url(/content/theme_main/img/cb.png)', 'background-repeat': 'no-repeat' });
    
                    } else {
                        cell.css({ 'background-image': 'url(/content/theme_main/img/cc.png)', 'background-repeat': 'no-repeat' });
    ...
    for (var i = 0; i < maxX; i++) {
        for (var j = 0; j < maxY; j++) {
            ...
            if (i == 0) {
                if (j == 0) {
                    cell.css({ 'background-image': 'url(/content/theme_main/img/rad.png)', 'background-repeat': 'no-repeat', 'background-position': '0px 0px' });

                }
                else if (j == maxY - 1) {
                    cell.css({ 'background-image': 'url(/content/theme_main/img/rad.png)', 'background-repeat': 'no-repeat', 'background-position': '0px -42px' });

                } else {
                    cell.css({ 'background-image': 'url(/content/theme_main/img/rad.png)', 'background-repeat': 'no-repeat', 'background-position': '0px -21px' });

                }
            }
            else if (i == maxX - 1) {
                if (j == 0) {
                    cell.css({ 'background-image': 'url(/content/theme_main/img/rad.png)', 'background-repeat': 'no-repeat', 'background-position': 'right 0px' });
                } else if (j == maxY - 1) {
                    cell.css({ 'background-image': 'url(/content/theme_main/img/rad.png)', 'background-repeat': 'no-repeat', 'background-position': 'right -42px' });

                } else {
                    cell.css({ 'background-image': 'url(/content/theme_main/img/rad.png)', 'background-repeat': 'no-repeat', 'background-position': 'right -21px' });


                }
            } else {
                if (j == 0) {
                    cell.css({ 'background-image': 'url(/content/theme_main/img/rad.png)', 'background-repeat': 'no-repeat', 'background-position': '-21px 0px' });
                } else if (j == maxY - 1) {
                    cell.css({ 'background-image': 'url(/content/theme_main/img/rad.png)', 'background-repeat': 'no-repeat', 'background-position': '-21px -42px' });

                } else {
                    cell.css({ 'background-image': 'url(/content/theme_main/img/rad.png)', 'background-repeat': 'no-repeat', 'background-position': '-21px -21px' });
...
function yandexStatToCart() {
    try {
        yaCounter47004228.reachGoal('vkorziny');
    } catch (e) {
    }
}
function yandexStatToOrder() {
    try {
        yaCounter47004228.reachGoal('oformitzakaz');
    } catch (e) {
    }
}
...
function loadMagnifier() {
    /*    $('.goods_pic.theme_main_preview').loupe();*/
...
function loadFilters() {
    ...
    $('.rng input').keyup(function (e) {
        
        var inps = $('input[parent="' + $(this).attr('parent') + '"]');
        if (parseInt(inps.filter('.from').val()) > parseInt(inps.filter('.to').val()) || isNaN(parseInt(inps.filter('.from').val())) || isNaN(parseInt(inps.filter('.to').val()))) {

            //var spl = $('#' + $(this).attr('parent')).val().split(';');
            //inps.filter('.from').val(spl[0]);
            //inps.filter('.to').val(spl[1]);
            return false;
        }

        $('#' + $(this).attr('parent')).slider("value", inps.filter('.from').val(), inps.filter('.to').val());

        //$('#' + $(this).attr('parent')).slider("option", "value", inps.filter('.from').val() + ";" + inps.filter('.to').val());
        //$('#' + $(this).attr('parent')).val(inps.filter('.from').val() + ";" + inps.filter('.to').val());
        //$('#' + $(this).attr('parent')).slider('update', { from: inps.filter('.from').val(), to: inps.filter('.to').val() })
...
function showFilteredPopup(obj) {
    ...
    $.post('/Master/ru/ClientCatalog/FilterCount', { args: text, pageID: $('#PageID').val(), objID: o }, function (d) {
        ...
        var html = '<div class="tooltip force horizontal"><a onclick="$(\'.tooltip.horizontal\').hide();" class="close-tooltip-force">X</a><span style="white-space:nowrap">Найдено: ' + d.Count + '</span><br><a href="' + (d.IsInCatalog == '1' ? link : d.Link) + '">Показать</a></div>';
        $('.tooltip.force.horizontal').remove();
        $('#btn-choose').attr('arg', link);
        obj.parents('.property').append(html);
        var dist = $(obj).offset().top - $(obj).parent().parent().parent().offset().top;
        if(dist>=0)
            $('.tooltip.force.horizontal').css('top', (dist) + "px");
    });
}
...
function loadSearch() {
    ...
    $(".search input[type='text']").each(function() {
        var item = $(this);
        item// don't navigate away from the field on tab when selecting an item
...
function loadCartBlock() {
    ...
    $('.compare input').change(function () {
        compare($(this).attr('arg'), $(this));
        return false;
    });
    $('.compare-prod a').click(function () {
        compare($(this).attr('arg'), $(this));
        $(this).parent().find('span').html($(this).attr('title'));
        return false;
    });
    $('.compare-prod span').click(function () {
        compare($(this).parent().find('a').attr('arg'), $(this).parent().find('a'));
        $(this).html($(this).parent().find('a').attr('title'));
        return false;
    });

    function compare(arg, obj) {
        var v = $.cookie('ForCompare');
        if (!v)
            v = '';
        var saved = v.split(';');
        ...
        if (saved.indexOf(arg) > -1) {
            ...
            $(obj).attr('title', 'Добавить в сравнение');
        } else {
            $(obj).parent().addClass('active');
            $(obj).attr('title', 'Убрать из сравнения');
...
function loadPopupUI() {
    ...
    loadMagnifier();
    //$('#el_modal_window .goods_pic.theme_main_preview').loupe();
...
function checkCalcRedirect() {
    if ($('#CalcPopup #NeedClose').val() == 'True') {
        setTimeout(function () { $('#CalcPopup .icon_close').click(); }, 2500);
    };
}

function showCalcOrder(hasMail) {

    $.get('/Master/ru/Forms/CalcPopup', { hasMail: hasMail, url : document.location.href }, function (d) {
        $('#el_modal_window').remove();
        $('body').append(d);
        $('body').find('input[type="file"]').each(function () {
            ...
            fu.fileupload({
                autoUpload: true,
                url: '/Master/ru/Forms/UploadFormFile',
                dataType: 'json',
                add: function (e, data) {
