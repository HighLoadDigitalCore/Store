...
function loadTree() {
    ...
    var dataLinkPartial = '/Master/ru/Home/getTreeDataPartial';

    $("#tree").jstree({
        "plugins": ["themes", "json_data", "ui", "crrm", "cookies", "dnd", "search", "types", "state"],
            ...
            "ajax": {
                ...
                "url": function (node) {
                    ...
                    if (node == -1) {
                        ...
                        nodeId = node.attr('id');
                        url = dataLinkPartial + "?nodeId=" + nodeId;
                    }
        ...
        */
        ...
        $('.jstree.jstree-default li.selected').removeClass('selected');
        $('.jstree.jstree-default li.jstree-selected').removeClass('jstree-selected');
        ...
        //$('#tree li.selected').trigger('click');
        ...
        console.log('opened: ' + $.cookie('node_opened_main'));
        var op = $.cookie('node_opened_main');
        if (op !== undefined) {
            var nodes = op.split(',');
            for (var node in nodes) {
                if ($(nodes[node]).find('>ul').length && $(nodes[node]).find('>ul').is(':hidden')) {
                    
                    $(nodes[node]).find('> ins').trigger('click');
                }
            }
        }
        $("#tree").show();
            ...
            if ($(this).parents('li').attr('id').indexOf('x') == 0 || $(this).parents('li').attr('uid') == '0') {
                ...
                if (catid == $(this).parents('li').attr('uid')) {
                    ...
                    */
                    ...
                    content.append('<a href="/" title="Добавить подраздел" onclick="return addCat(' + $(this).parents('li').attr('uid') + ')" class="addpage"></a>');
                    content.append('<a href="/" title="Редактировать раздел" onclick="return editCat(1)" class="editpage"></a>');
                    content.append('<a href="/" title="Список подразделов" onclick="return catList(' + $(this).parents('li').attr('uid') + ')" class="ico-list"></a>');
                    $(this).append(content);
...
#1#
                    ...
                    var content = $('<span class="btns"></span>');
                    content.append('<a href="/" title="Удалить товар" onclick="return deleteProd(' + $(this).parents('li').attr('uid') + ')" class="delpage"></a>');
                    /*content.append('<a href="/" title="Редактировать товар" onclick="return editProd(' + $(this).parents('li').attr('uid') + ')" class="editpage"></a>');#1#
                    content.append('<a href="/" title="Открыть сайт" target="_blank" onclick="return openPage(&quot;' + $(this).parents('li').attr('id') + '&quot;, this)" class="open-page ico-eye"></a>');
...
function deleteCatalogVideo(categoryid, type) {
    var res = confirm("Удалить все видео для этой категории?");
    if (res) {
        $.get('/Master/ru/Home/DeleteCategoryVideo', { CategoryID: categoryid, type:type }, function (data) {
...
function saveText() {
    ...
    if ($('#dialog-text-edit').attr('cmspageid') && $('#dialog-text-edit').attr('viewid')) {
...
function loadUIElems() {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                for (var j = 0; j < data[i].length; j++) {
                    ...
                    if (i == 0) {
                        ...
                        if (data[i][j] != "checkbox") {
                            html += '<input type="text" old="' + data[i][j] + '" value="' + data[i][j] + '">';
                        } else {
    ...
    function loadTableEdit() {
        ...
        $('.inline-edit #add-line').click(function () {
            $(this).parents('table').find('.last-line').before('<tr><td><input type="text" value="" old=""></td><td><input style="display:none" type="checkbox"></td><td><input type="text" old="" value=""></td><td><input style="display:none" type="checkbox"></td><td style="text-align:center"><input type="checkbox"></td><td><input type="text" value=""></td><td><a href="#nogo" id="del-line">удалить</a></td></tr>');
            loadTableDelete();
...
function loadOrderBoxes() {
    ...
    $('a[box="orderboxDown"], a[box="orderboxUp"]').click(function () {
        debugger;
        var me = $(this);
        var another = me.attr('box') == "orderboxDown" ? me.closest('tr').next().find('[box="orderboxDown"]') : me.closest('tr').prev().find('[box="orderboxUp"]');
        var bx = me.closest('div').find('input[box="orderbox"]');
        var target = another.closest('div').find('input[box="orderbox"]');
        if (bx.length && target.length) {
            bx.val(target.val());

            var link = '/Master/ru/' + bx.attr('target') + "/" + bx.attr('action');
            var query = $('#Request').val().substr(1);
            $.post(link, { id: bx.attr('arg'), value: bx.val(), page: bx.attr('page'), tablename: bx.attr('tablename'), uidname: bx.attr('uidname'), ordername: bx.attr('ordername'), cc: bx.attr('cc'), ca: bx.attr('ca'), addqs: bx.attr('addqs'), query: query }, function (d) {
                if (d.length) {
                    var table = bx.closest('table');
                    table.find('tr').each(function () {
                        if (!$(this).find('th').length)
                            $(this).remove();
                    });
